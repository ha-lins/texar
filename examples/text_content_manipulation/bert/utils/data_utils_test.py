...
class SSTProcessor(DataProcessor):
	...
	def get_test_examples(self, data_dir):
		"""See base class."""
		return self._create_examples(
			self._read_tsv(os.path.join(data_dir, "new_test_for_accu.tsv")), "test")
	...
	def _create_examples(self, lines, set_type):
		...
		examples = []
		if set_type == 'train' or set_type == 'dev' or set_type == 'test':
	...
	def make_Vocab(self, data_dir):
		stages = ['train', 'dev', 'test.adv2.sent']
		global all_vocab

		vocabs = {}
		for idx, stage in enumerate(stages):
			file_name = os.path.join(data_dir, '{}.tsv'.format(stage))
			#		for idx, stage in enumerate(stages):
			print('===============file name is {}'.format(file_name))
			vocabs[stage] = make_vocab(file_name)
			# print('====================vocabs[{}] is {}:'.format(stage, vocabs[stage]))
			all_vocab.update(vocabs[stage])
		with open(os.path.join(data_dir, 'all.vocab.txt'), 'w') as vocab_file:
			vocab_file.write(u'\n'.join(all_vocab))
...
def file_based_convert_examples_to_features(
	...
	with open(os.path.join('data/SST-2', 'all.vocab.txt'), 'a') as vocab_file:
		vocab_file.write(u'\n'.join(all_vocab))
...
def prepare_TFRecord_data(processor, tokenizer,
	...
	#print('=================test_examples is {}'.format(test_examples))
	test_file = os.path.join(output_dir, "test_for_accu.tf_record")
	file_based_convert_examples_to_features(
